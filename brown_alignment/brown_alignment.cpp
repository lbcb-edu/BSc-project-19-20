...
namespace brown {
    ...
    void init_matrix(const char* query, unsigned int query_length,
        ...
        if (type == AlignmentType::global) {
            ...
            {
                matrix[i][0].value = gap*i;
                matrix[i][0].parent = ParentType::left;                
            }
    ...
    int global_alignment(const char* query, unsigned int query_length,
        ...
        vector<vector<matrixCell>> matrix(query_length + 1, vector<matrixCell>(target_length + 1));
    ...
    int pairwise_alignment(const char* query, unsigned int query_length,
        ...
        case AlignmentType::global:
            return global_alignment(query, query_length, target, target_length, type, match, mismatch, gap);
        case AlignmentType::local:
            return local_alignment(query, query_length, target, target_length, type, match, mismatch, gap);
        case  AlignmentType::semi_global:
            return semi_global_alignment(query, query_length, target, target_length, type, match, mismatch, gap);
        default:
